...
class UXDesignApp extends BaseDifyApp {
    constructor() {
        ...
        this.actualApiKeyId = 'app-FBfPAeUwGK3rkBWRXwbx'; // 从映射表中获取的实际ID
    ...
    async _loadApiConfig() {
        console.log(`[UXDesignApp] 使用直接方法获取API配置...`);
        ...
        try {
            ...
            // 3. 非常宽松的匹配 - 包含"ux"的任何记录
            if (!apiKeyRecord) {
                apiKeyRecord = apiKeys.find(key => 
                    key.applicationID && 
                    (key.applicationID.toLowerCase().includes('ux') || 
                     key.applicationID.toLowerCase().includes('fbf')));
            }
            ...
            console.error(`[UXDesignApp] 加载API配置时出错:`, error);
            if (this.ui) {
