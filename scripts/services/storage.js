...
/**
 * 创建或更新当前登录用户的设置.
 * @param {object} settings - 包含 role, apiKeys, 和可选的 language 的对象.
 * @param {string} settings.role - 用户角色.
 * @param {object} settings.apiKeys - 包含 API 密钥的对象.
 * @param {string} [settings.language] - 用户语言偏好 (e.g., 'zh-CN', 'en').
...
export async function saveCurrentUserSetting(settings) {
    try {
        ...
        const inputData = {
            ...
            language: settings.language, // Include language, pass undefined if not provided
            apiKeys: {
        ...
        console.log("Attempting to save UserSettings for user:", userId, "Data:", inputData);

        // Fetch existing settings ONLY to determine create vs update for role/apiKeys
        // We will always attempt to update language if provided, even if record exists
        ...
        // Clean inputData: remove language if it wasn't explicitly provided in the settings object
        // This prevents accidentally overwriting an existing language setting with undefined
        // when only updating role or apiKeys.
        ...
        if (existingSettings && existingSettings.data.getUserSettings) {
            ...
            console.log('Updating UserSettings:', inputData);
             // Ensure we don't send the ID in the input for update mutation if schema expects it implicitly
             const updateInput = { ...inputData };
             delete updateInput.id; // Remove id if update mutation doesn't need it in input

            result = await client.graphql({
                query: mutations.updateUserSettings, // Assumes mutation name generated by Amplify
                // variables: { input: inputData } // Original
                variables: { input: { id: userId, ...updateInput } } // Ensure ID is top-level if needed
            });
            ...
            console.log('Creating UserSettings:', inputData);
             // Make sure language has a default if creating and not provided
             if (inputData.language === undefined) {
                 inputData.language = 'zh-CN'; // Default language on creation
             }
            result = await client.graphql({
