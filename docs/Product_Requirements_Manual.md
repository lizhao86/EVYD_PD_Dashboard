...
| 1.6.25 | 2025-05-05 | EVYD产品团队 | **聊天界面功能全面升级**: 实现聊天历史记录存储功能(基于DynamoDB)、历史对话加载、Markdown格式渲染和对话管理功能。优化对话气泡样式设计，改善视觉体验。添加对话重命名功能，允许用户自定义对话标题。实现智能对话切换，当删除当前对话时自动切换到最新对话。修复了多项交互问题，包括对话加载错误处理、Markdown渲染一致性和重复对话创建的问题。提升了整体聊天体验的稳定性和易用性。 |
| 1.6.26 | 2025-05-06 | EVYD产品团队 | **通用聊天界面交互优化**: 实现"停止响应"按钮功能。该按钮在AI流式响应时显示在输入框上方，点击可中断当前的Dify API请求。按钮UI按照新设计稿实现，确保视觉一致性。 |
| 1.6.27 | 2025-05-07 | EVYD产品团队 | **API URL构建修复**: 修复API URL构建问题，解决生产环境中的404错误。修复了`modules/common/BaseDifyChatApp.js`、`modules/common/base-dify-app.js`和`modules/common/dify-client.js`中的硬编码API路径，确保API请求能正确发送到配置的服务器URL。解决了本地环境测试正常但生产环境失败的问题。 |
...
#### 4.2.2 通用聊天界面 (原用户手册生成器)
- **(已重构)** 原"用户手册生成器"已重构为通用的**聊天界面**，基于 `BaseDifyChatApp.js` 和 `ChatUIManager.js` 框架。
- **核心功能**:
    - 与 Dify 对话型应用 API (Chat API) 交互，支持流式响应。
    - 显示用户和 AI 消息，支持 Markdown 渲染。
    - 提供消息操作按钮：复制、重新生成、反馈 (点赞/点踩)。
    - **历史记录**:
        - 对话自动保存到 AWS DynamoDB (通过 AppSync GraphQL)。
        - 加载历史对话列表和消息内容。
        - **对话管理**: 支持删除对话 (删除后智能切换到最新对话或新建对话)。
        - **对话重命名**: 允许用户修改对话标题。
    - **建议问题**: 支持显示和点击 Dify 返回的建议问题。
    - **停止响应**: 新增"停止响应"按钮，在AI生成过程中显示在输入框上方，点击可中断当前生成过程。
    - **UI/UX 优化**:
        - 优化聊天气泡样式（侧边三角形）。
        - 按钮精确定位。
        - 修复了 Markdown 渲染、按钮重复监听、Dify 会话 ID 同步、对话自动创建等多个 Bug。
- 完成输出后展示每次生成的耗时，Token消耗 (由 Dify API 提供)。
- **(已修复)** 优化了消息操作按钮（复制、重试、反馈）的定位和重新生成逻辑。

##### 4.2.2.1 Dify 对话型应用 (Chat API) 原理
   - **应用类型**: Dify Chat API 应用
   - **交互模式**: 通过 `POST /chat-messages` 发送用户消息，通过 Server-Sent Events (SSE) 接收 AI 的流式回复。
   - **核心模型**: 取决于具体配置的 Dify 对话型应用，例如之前用户手册使用的 Gemini 模型 (langgenius/gemini/google:gemini-2.0-flash-thinking-exp-01-21)。
   - **上下文管理**: 通过 Dify 返回的 `conversation_id` 维护对话上下文。前端需将此 ID 与 DynamoDB 中的对话记录关联。
   - **附加功能**:
       - `POST /messages/:message_id/feedbacks`: 发送消息反馈。
       - `GET /messages/{message_id}/suggested`: 获取建议问题。
       - **对话命名**: Dify 可根据对话内容自动生成名称 (通过 API 响应的 `metadata.name` 获取)。

##### 4.2.2.2 待办事项 (TODO for tomorrow - 2025-04-30) (已部分实现)
   - **完善反馈机制**:
       - 前端已实现点击发送反馈到 Dify API (`POST /messages/:message_id/feedbacks`)。 (已完成)
       - **(待办)** 将反馈结果（成功/失败，或更详细的统计）存储到后端 (如 DynamoDB `UserSettings` 或专门的反馈表) 用于分析。
