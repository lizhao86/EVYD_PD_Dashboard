...
2.  **创建通用 API 客户端 (`/modules/common/dify-client.js`) (待办):**
...
4.  **~~重构各 AI 应用:~~ (UI 层已于 V1.6.13 完成)**
    -   ~~修改 `requirement-analysis/index.js`, `user-manual/index.js`, `user-story/index.js`, `ux-design/index.js`，使其继承 `BaseDifyApp`。~~ (改为使用通用UI模块)
    -   ~~移除或大幅简化各应用独立的 `api.js` 和 `ui.js` 文件。~~ (ui.js 已移除, api.js 待进一步整合) 